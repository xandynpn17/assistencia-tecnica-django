...
PAPEIS_TECNICOS_PADRAO = ("tecnico",)


def _q_usuario_tecnico():
    return Q(tipo_usuario__in=PAPEIS_TECNICOS_PADRAO) | Q(atua_como_tecnico=True) | Q(is_superuser=True)


def usuarios_tecnicos_qs(*, empresa=None):
    user_model = get_user_model()
    queryset = user_model.objects.filter(is_active=True).filter(_q_usuario_tecnico())
...
def usuario_apto_tecnico(usuario):
    ...
    return bool(
        ...
        or getattr(usuario, "atua_como_tecnico", False)
    )
...
def filtro_sem_tecnico(prefixo="tecnico_responsavel"):
    ...
    tecnico_invalido_q = (
        ...
        & Q(**{campo("atua_como_tecnico"): False})
