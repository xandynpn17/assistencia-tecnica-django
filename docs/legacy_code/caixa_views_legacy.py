...
def dashboard_caixa(request):
    ...
    custos_fixos_previsto_mes = (
        custos_fixos_mes_qs.exclude(status="cancelado").aggregate(total=Sum("valor_previsto"))["total"]
        or Decimal("0.00")
    )
    custos_fixos_pago_mes = (
        custos_fixos_mes_qs.exclude(status="cancelado").aggregate(total=Sum("valor_pago"))["total"]
        or Decimal("0.00")
    )
...
def custos_fixos(request):
    ...
    base_url = reverse("caixa:custos_fixos")
    ...
    form = CustoFixoMensalForm(
        instance=item_edicao,
    ...
    custos_qs = (
        CustoFixoMensal.objects.select_related("centro_custo")
    ...
    anos_disponiveis = sorted(anos_db.union({ano}), reverse=True)
